...
#ifdef Q_OS_WASM
    // On wasm the browser picker delivers file contents asynchronously.
    RS_DIALOGFACTORY->requestImageOpenContent(
        [this](const QString &fileName, const QByteArray &content) {
            ...
            const QString tempPath = "/tmp/" + baseName;
            {
                QFile f(tempPath);
            ...
            m_imageData->data.file = tempPath;
            m_imageData->img = QImage(m_imageData->data.file);
            if (m_imageData->img.isNull()) {
                setFinished();
                return;
            }
