...
void loadTranslations() {
    ...
    LC_GROUP("Appearance");
    QString lang = LC_GET_STR("Language", "ru");
    QString langCmd = LC_GET_STR("LanguageCmd", "ru");
    LC_GROUP_END();
...
#    else
...
#if defined(__EMSCRIPTEN__) && defined(__WASM_EXCEPTIONS__)
    // Async main (JSPI + native wasm exceptions build): with -fwasm-exceptions
    // QApplication::exec() is unsupported, so main() returns early and the
    // browser drives the Qt event loop. The application object must therefore
    // outlive main() -- allocate it on the heap and intentionally leak it
    // (the QC_ApplicationWindow singleton is likewise kept alive). Using a
    // reference keeps every downstream `app.` usage unchanged.
    LC_Application& app = *(new LC_Application(argc, argv));
#else
    ...
    initSystem(argv, app);
#else
    // On wasm there are no CLI args; argv[0] is unreliable, so initialise
    // the system with a safe default program dir instead of resolving it
    // from argv[0] (which may be "/" or empty).
...
#else
    std::unique_ptr<QSplashScreen> splash; // no splash screen on wasm
#endif
...
#else
    // On wasm native file dialogs are not available; always use Qt-drawn dialogs.
...
#else
    // On wasm, no files to load from CLI
...
#else
    LC_GROUP("Startup");
    { if (first_load){ LC_SET("FirstLoad", false); } }
    LC_GROUP_END();
#endif

#if defined(__EMSCRIPTEN__) && defined(__WASM_EXCEPTIONS__)
    // JSPI + native wasm exceptions build: async main.
    //
    // QApplication::exec() is not usable here: on wasm it routes through
    // QEventDispatcherWasm::handleApplicationExec(), which calls
    // emscripten_set_main_loop(..., simulateInfiniteLoop=true). That returns
    // control to the browser by throwing a JS "unwind" exception, which is
    // incompatible with -fwasm-exceptions (hence the Qt docs' "exec() is not
    // supported with exceptions" note). Instead we return from main() and let
    // the browser drive Qt's event loop (Emscripten keeps the runtime alive as
    // main() is not the process exit). `app` was heap-allocated above so it
    // outlives main().
    //
    // JSPI suspends (QDialog::exec(), combobox popups, nested event loops) work
    // because every wasm entry point that can reach a suspend is wrapped as a
    // WebAssembly.promising frame: DOM events via EventListener::handleEvent
    // (registered with emscripten::async(); see the Qt qstdweb.cpp patch in
    // .wasm/Dockerfile.jspi) and timer/posted-event callbacks via the
    // emscripten_set_timeout/emscripten_async_call promising wrap applied to the
    // generated .js (see .wasm/jspi_postprocess.py). Unlike classic one-level
    // Asyncify, JSPI allows arbitrary suspend nesting.
    RS_DEBUG->print("main: async-main (JSPI) -- browser drives the event loop");
    return 0;
#else
