    ...
    if (contextMenu != nullptr) {
        if (!contextMenu->isEmpty()) {
            ...
            if (actions.size() == 1) {
                ...
                // On the Emscripten/wasm (Asyncify) platform, QMenu::exec()
                // runs a *nested* event loop. Popup submenus that are created
                // inside that nested loop never receive mouse move/click events
                // (only keyboard), so second-level context menus can't be used
                // with the mouse. Show the menu non-blocking with popup() so it
                // and its submenus run in the main event loop, where mouse
                // events are delivered correctly. The menu deletes itself on
                // close, so we must not delete it below.
...
void QG_GraphicView::adjustOffsetControls(){
    ...
    // Re-entrancy guard: on single-threaded wasm, std::mutex is a no-op
    // stub, so the try_lock() guard below never trips. Use a boolean flag
    // to break the feedback loop:
    //   adjustOffsetControls → slotHScrolled → setOffsetX →
    //   fireViewportChanged → onViewportChanged → adjustOffsetControls
    if (m_inAdjustOffsetControls)
        return;
    m_inAdjustOffsetControls = true;

    std::unique_lock<std::mutex> lock(m_scrollbarMutex, std::defer_lock);
    if (!lock.try_lock()) {
        m_inAdjustOffsetControls = false;
        return;
    }
