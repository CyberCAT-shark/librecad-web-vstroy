...
//                        RS_Document* doc=NULL) = 0;
    ...
    *              attributes.
    ...
    /**
     * Async variant for platforms (e.g. wasm) where the file picker delivers
     * the selected file's contents asynchronously instead of a path. The
     * implementation invokes @p ready with the file name and content bytes,
     * or with an empty byte array if the user cancels. The default is a no-op
     * so non-wasm callers continue to use requestImageOpenDialog().
     */
    virtual void requestImageOpenContent([[maybe_unused]] const std::function<void(const QString &, const QByteArray &)> &ready) {};
