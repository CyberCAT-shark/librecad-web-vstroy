...
#ifdef Q_OS_WASM
    // On wasm there is no host filesystem; fonts and patterns are bundled into
    // MEMFS at build time via Emscripten's --preload-file (see CMakeLists.txt).
    // Expose them at the same path the bundle is mounted under.
    dirList.append("/librecad/support/" + subDirectory);
#endif
