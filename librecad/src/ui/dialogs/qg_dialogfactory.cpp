...
/**
 * Async (wasm) image picker: invokes the browser file picker via
 * QFileDialog::getOpenFileContent and delivers the selected file's name and
 * contents through @p ready. The filter is built from the supported image
 * formats. On non-wasm builds this is unused (callers use
 * requestImageOpenDialog()).
 */
void QG_DialogFactory::requestImageOpenContent(const std::function<void(const QString &, const QByteArray &)> &ready) {
#ifdef Q_OS_WASM
    QStringList filters;
    QString all;
    bool haveJpeg = false;
    for (const QByteArray &format: QImageReader::supportedImageFormats()) {
        if (format.toUpper() == "JPG" || format.toUpper() == "JPEG") {
            ...
            filters.append(QString("%1 (*.%1)").arg(QString::fromLatin1(format)));
            all += QString(" *.%1").arg(QString::fromLatin1(format));
        }
    }
    ...
    QFileDialog::getOpenFileContent(filter, [ready](const QString &fileName, const QByteArray &content) {
        ready(fileName, content);
    });
#else
    if (ready) ready(QString(), QByteArray());
#endif
}
