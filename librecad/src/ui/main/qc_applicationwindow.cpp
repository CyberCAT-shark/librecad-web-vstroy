...
#ifdef Q_OS_WASM
// Defined below: browser file picker -> MEMFS -> onReady(path). Used by the
// file-open/new-from-template/import slots (see the definition for why Qt's
// getOpenFileContent() is bypassed on the JSPI build).
static void wasmPickFileToMemfs(std::function<void(const QString &)> onReady);
#endif

void QC_ApplicationWindow::slotFileNewFromTemplate() {
#ifdef Q_OS_WASM
    wasmPickFileToMemfs([this](const QString &path) {
        QC_MDIWindow *w = nullptr;
        newDrawingFromTemplate(path, w);
    });
#else
 ...
 */
#ifdef Q_OS_WASM
// On the JSPI wasm build, Qt's QFileDialog::getOpenFileContent() delivers an
// unfilled (garbage) buffer -- its chunked FileReader content copy does not
// populate the QByteArray. Instead we pick the file in JavaScript, read its
// bytes there and write them straight into the Emscripten MEMFS filesystem
// (FS.writeFile, which copies correctly), then open by path.
namespace {
// Set immediately before triggering the JS picker; invoked (queued onto the Qt
// event loop, i.e. a WebAssembly.promising frame) once the file is in MEMFS.
std::function<void(const QString &)> g_wasmFileReadyCb;
}

extern "C" EMSCRIPTEN_KEEPALIVE void lc_wasm_file_ready(const char *cpath)
{
    const QString path = QString::fromUtf8(cpath);
    auto cb = g_wasmFileReadyCb;
    QMetaObject::invokeMethod(qApp, [path, cb]() {
        if (cb) cb(path);
    }, Qt::QueuedConnection);
}

// Show a browser file picker, copy the chosen file into MEMFS under /tmp, then
// call onReady(path) on the Qt event loop. showOpenFilePicker() is invoked
// synchronously at the start of the async function so it inherits the click's
// user activation; a hidden <input> is used as a fallback.
static void wasmPickFileToMemfs(std::function<void(const QString &)> onReady)
{
    g_wasmFileReadyCb = std::move(onReady);
    EM_ASM({
        (async function() {
            let file = null;
            try {
                if (typeof window.showOpenFilePicker === 'function') {
                    ...
                    file = await new Promise(function(resolve) {
                        ...
                        inp.type = 'file';
                        inp.accept = '.dxf,.DXF,.dwg,.DWG,.cxf,.lff,.jww';
            ...
            catch (e) { console.error('[LibreCAD] reading file failed', e); return; }
            const path = '/tmp/' + file.name;
            try {
                try { FS.mkdir('/tmp'); } catch (e) { /* exists */ }
                FS.writeFile(path, buf);
            } catch (e) { console.error('[LibreCAD] FS.writeFile failed', e); return; }
            Module.ccall('lc_wasm_file_ready', null, ['string'], [path]);
        })();
    });
}
#endif
...
#ifdef Q_OS_WASM
    wasmPickFileToMemfs([this](const QString &path) {
        openFile(path, RS2::FormatUnknown);
    });
#else
...
#if !defined(LC_NO_PRINT) || defined(Q_OS_WASM)
        using namespace LC_Printing;
#  ifdef Q_OS_WASM
        // Wasm has no QPrinter/QPrintDialog (no PrintSupport); render the
        // drawing to a QPdfWriter-backed QByteArray and trigger a browser
        // download.
        LC_Printing::WasmExportPDF(*w);
#  else
...
#ifdef Q_OS_WASM
    // Pick the file in the browser, copy it into MEMFS, then feed the path into
    // the library-insert action. (Qt's getOpenFileContent() delivers a garbage
    // buffer on the JSPI build -- see wasmPickFileToMemfs.)
    wasmPickFileToMemfs([this](const QString &tempPath) {
        if (m_actionHandler == nullptr) return;
