...
#ifdef Q_OS_WASM
    // Modal exec()-based save dialogs are not available on wasm; the wasm save
    // path serializes to MEMFS and triggers a browser download instead, so this
    // helper is never reached on wasm. Return an empty result for safety.
