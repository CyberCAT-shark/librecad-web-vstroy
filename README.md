# LibreCAD (experimental Web/WebAssembly fork)

> ⚠️ **This is an experimental fork of LibreCAD that compiles the full
> application to WebAssembly so it runs in a browser.** It is **not** an
> official LibreCAD project. For the real thing, see
> [LibreCAD/LibreCAD](https://github.com/LibreCAD/LibreCAD).
>
> **Live demo:** https://magik.net/librecad
>
> The port builds Qt 6.9 from source for the `wasm-emscripten` platform with
> **JSPI** (`QT_EMSCRIPTEN_ASYNCIFY=2`) + native Wasm exceptions, which lets
> LibreCAD's modal dialogs, combo-box drop-downs and nested colour pickers work
> in the browser (classic Emscripten Asyncify only supports one level of
> suspend). All web-specific changes live behind `#if defined(Q_OS_WASM)` /
> `#ifndef LC_NO_PRINT` / `#ifndef LC_NO_NETWORK` guards, so the desktop build
> is unaffected. Build tooling and notes are under [`.wasm/`](.wasm/).
>
> **Status:** experimental. Requires a recent Chromium-based browser
> (Chrome/Edge 137+); Firefox and Safari don't ship JSPI yet. Work is on the
> `wasm-port` branch.
