...
#ifdef Q_OS_WASM
    // Modal exec()-based file dialogs are not available on wasm; file content is
    // delivered asynchronously via QFileDialog::getOpenFileContent instead.
    // Callers that still route through this method are no-ops on wasm (handled
    // at their own entry points); return "" to indicate cancellation.
    return QString();
#else
...
#ifdef Q_OS_WASM
    // Modal exec()-based file dialogs are not available on wasm; saving on wasm
    // serializes to a MEMFS temp file and triggers a browser download instead.
    return QString();
#else
