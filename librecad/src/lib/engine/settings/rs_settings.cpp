...
#ifdef Q_OS_WASM
    // On wasm there is no persistent host filesystem. RS_Settings writes to an
    // explicit INI file under a directory that is mounted as IDBFS by the
    // Emscripten --pre-js (see .wasm/prejs.js), so settings survive page
    // reloads. If the IDBFS mount failed, this still works against ephemeral
    // MEMFS (no persistence, but no crash either).
    const QString settingsDir = "/home/web_user/.local/share/LibreCAD";
    QDir().mkpath(settingsDir);
    auto* settings = new QSettings(settingsDir + "/settings.ini", QSettings::IniFormat);
#else
