...
#ifdef Q_OS_WASM
#include "lc_wasm_pdf_export.h" // lc_wasm::downloadData
#endif
...
#ifdef Q_OS_WASM
        // On wasm the filesystem is an ephemeral in-memory MEMFS, so "Save"
        // must always hand the file back to the user as a browser download
        // (using the current name, if any, as the hint) -- writing silently to
        // MEMFS would be lost on reload.
...
bool LC_DocumentsStorage::doSaveGraphicAs(RS_Graphic* graphic, RS_GraphicView *graphicView, bool &cancelled, const QString& currentFileName){
#ifdef Q_OS_WASM
    // No native file picker on wasm: serialize to a MEMFS temp file and trigger
    // a browser download via QFileDialog::saveFileContent.
    cancelled = false;
    QString baseName = QFileInfo(currentFileName).completeBaseName();
    if (baseName.isEmpty()) baseName = "untitled";
    const QString downloadName = baseName + ".dxf";
    const QString tempPath = "/tmp/" + downloadName;

    QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
    graphic->setGraphicView(graphicView);
    bool result = saveGraphicAs(graphic, tempPath, RS2::FormatDXFRW, true);
    QApplication::restoreOverrideCursor();
    ...
    if (result) {
        ...
        if (f.open(QIODevice::ReadOnly)) {
            ...
            f.close();
            // Download via JS Blob + <a download> rather than
            // QFileDialog::saveFileContent(), which on the JSPI build prefers
            // showSaveFilePicker() + a chunked writable-stream write that does
            // not reliably deliver the bytes.
            lc_wasm::downloadData(data, downloadName);
        }
    }
    return result;
#else
